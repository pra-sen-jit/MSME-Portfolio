...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.query('SELECT * FROM users WHERE artisanId = ? OR PhoneNumber = ?', [artisanId, PhoneNumber]);
        if (rows.length > 0) {
        ...
        await db.query("INSERT INTO users (username, artisanId, PhoneNumber, password) VALUES (?, ?, ?, ?)", 
